...
if ($rootPath === false || ! is_file($rootPath . DIRECTORY_SEPARATOR . 'vendor' . DIRECTORY_SEPARATOR . 'autoload.php')) {
    throw new RuntimeException('Unable to resolve the project root for breadcrumb tests.');
}

$publicPath = rtrim(sys_get_temp_dir(), '\\/') . DIRECTORY_SEPARATOR . 'ci4-breadcrumb-test-public';

if (! is_dir($publicPath) && ! mkdir($publicPath, 0775, true) && ! is_dir($publicPath)) {
    throw new RuntimeException('Unable to create the test public directory for breadcrumb tests.');
}
...
if ($packageMode) {
    ...
    if (! is_dir($configPath) && ! mkdir($configPath, 0775, true) && ! is_dir($configPath)) {
        throw new RuntimeException('Unable to create the test config directory for breadcrumb tests.');
    }
    ...
    foreach ([$writablePath, $writablePath . DIRECTORY_SEPARATOR . 'cache', $writablePath . DIRECTORY_SEPARATOR . 'logs', $writablePath . DIRECTORY_SEPARATOR . 'session', $writablePath . DIRECTORY_SEPARATOR . 'uploads'] as $directory) {
        if (! is_dir($directory) && ! mkdir($directory, 0775, true) && ! is_dir($directory)) {
            throw new RuntimeException('Unable to create the writable directory for breadcrumb tests.');
        }
    }
